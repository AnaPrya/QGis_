import os
import pandas as pd
from qgis.core import QgsProject, QgsVectorLayer, QgsField, QgsFeature, QgsExpression, QgsExpressionContext, QgsExpressionContextUtils
from PyQt5.QtCore import QVariant

# Caminhos para as pastas
caminho_propriedades = r"C:\propriedades"
caminho_alertas = r"C:\alertas"
...
if camada_propriedades.isValid() and camada_alertas.isValid():
    ...
    for feature in camada_saida.getFeatures():
        ...
        # Essa parte do 'número de lote' foi colocada para se adequar ao banco de dados usado quando fiz o código. Nem todo banco tem isso.
    ...
    camada_saida_data = []
    for feature in camada_saida.getFeatures():
        camada_saida_data.append({
            'Proprietário': feature['layer'],
            'Número do Lote': feature['Nome'],
            'Área da Sobreposição': feature['AREA_M'],
            'ALERTM': feature['ALERTM']
        })

    # Criar DataFrame com os dados
    df = pd.DataFrame(camada_saida_data)

    # Caminho para salvar o arquivo Excel
    caminho_excel = r"C:\Users\abartolo\Documents\resultado_analise.xlsx"

    # Salvar o DataFrame no arquivo Excel usando xlsxwriter
